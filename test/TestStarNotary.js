...
it('can add the star name and star symbol properly', async() => {
    ...
    let tokenId = 15;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello!', tokenId, {from: accounts[0]})
    assert.equal(await instance.name(), 'ITACHI');
    assert.equal(await instance.symbol(), 'ITI');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId2 = 26;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello Id1!', tokenId1, {from: accounts[1]});
    await instance.createStar('Hello Id2!', tokenId2, {from: accounts[2]});
